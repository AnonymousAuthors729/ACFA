...
Low-end embedded devices are increasingly used in a wide range of ``smart'' applications and spaces. They are implemented under strict cost and energy budgets, using microcontroller units (MCUs) that lack  security features akin to those available in general-purpose processors.(featured in smartphones and personal computers).
